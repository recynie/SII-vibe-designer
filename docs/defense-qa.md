...
**自适应**：实测显示 critic 在文案任务上**完全换了维度**——例如 Foundry Lab slogan 评审用「克制 / 动手气质 / 精准 / 记忆点 / 调性统一」（见 `run-foundry-copy/artifacts/copy/v1-slogan-review.md` 42/50），品牌简介用「精准 / 受众契合 / 调性 / 可读性 / 完整性」。这是 LLM 推理结合任务类型的合理行为——`critic.md` 现在显式允许文案换轴，反 slop 警觉作为隐性原则。

**为什么是 5 个 + 反 slop 隐性，不是别的**：5 维度足够覆盖一个产物的关键质量切面又不至于膨胀。反 slop 不写成显式维度，是因为它在不同任务上的体现不一致（紫渐变只跟视觉相关，"赋能/打造"只跟文案相关），让 critic 按任务推断更精准。
